...
1. **Registration via Invite Link**
   ...
   - User opens the link, sets password, name, and completes registration.
...
2. **Login**
   - Registered user enters a an email address and password.
...
3. The system must allow password resets only via valid, unique password reset links.
4. Each password reset link must be single-use and expire after a configurable period.
5. Registration requires setting and confirming a password and setting a name.
6. The system must enforce that email adresses are unique across all users.
7. The system must authenticate users using email address and password, issuing JWT access and refresh tokens upon successful login.
8. Access tokens must be short-lived; refresh tokens must be longer-lived and allow obtaining new access tokens.
9. The system must validate access tokens for protected resource requests.
10. The system must validate refresh tokens and issue new access tokens when requested.
11. The system must prevent registration with invalid or expired invite links and provide clear error messaging.
12. The system must allow admins to generate invite links for new users.
13. The system must allow admins to generate password reset links.
14. Emailing invite links and handling delivery failures is explicitly out of scope.
15. The system must support up to 10,000 registered users and 100 requests/sec.
16. The system must apply rate limiting to registration and login attempts (e.g., 5 attempts per minute per IP) to prevent abuse.
17. The system must keep a count of failed login requests for a user.
18. The system must reset the count of failed login request for a user upon successful login.
19. The system must be implemented using .NET.

## Success Criteria

- 100% of registrations occur only via valid invite links.
- 100% of password resets occur only via valid password reset links.
- 100% of invite links are single-use and expire as configured.
- 100% of password reset links are single-use and expire as configured.
...
- User: emailaddress (unique), name, password (hashed), account status (invited, registered, passwordresetrequired), failed login attempts
